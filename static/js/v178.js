...
function WidthChange(mq){
    ...
    if(!(bool) || ((we/he)<1)){
        console.log("not ok");
        alert("This webapp is viewable in pc or  desktop.. ")
        location.reload();
...
function edit_row(no){
	...
	for(i=0;i<l.length;i++){
		...
		var y
		y=`row_no${no}`;
		k.innerHTML="<input type='text' class='edit_data"+no+"' value='"+l[i].innerText+"' style='width:90px;padding:2px'>";
	}
...
function tryaj(){
	...
	$.post("/pie",{"array":`${arr}`,"rowlen":`${tableheads.length-1}`,"arrlen":`${arr.length}`},function(data,status)
            {
            	 var z= document.getElementsByClassName("nextf");
				    if(z.length==0){
