...
def viewresult(name):
    ...
    csm=s.columns
    if("x com" not in csm):
        s["x com"]=[0 for i in range(len(s))]
    if("labavil" not in  csm):
        s["labavil"]=[0 for i in range(len(s))]
    ...
    for j in range(len(cla)):
        ...
        for i in range(len(s)):
            if(s["cla"][i]==cla[j]):
                ...
                l.append([s["theorey"][i],s["Course Teachers"][i],s["lab"][i],s["x com"][i]])
    ...
    for j in range(len(t)):
        ...
        for i in range(len(s)):
            if(s["Course Teachers"][i]==t[j]):
                ...
                l.append([s["theorey"][i],s["cla"][i],s["lab"][i],s["x com"][i]])
        l.sort()
        d1.update({t[j]:[l]})
    sch=[]
